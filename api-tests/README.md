...
O enunciado já define os 3 endpoints a testar; a escolha aqui foi em como validar cada um. Pra cada endpoint dava pra só checar o status 200 e seguir em frente, mas isso não pega muita coisa, então entrou validação de contrato (schema) em todos, e o caminho negativo no endpoint que faz mais sentido ter um (raça que não existe).
